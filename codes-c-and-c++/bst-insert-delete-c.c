...
void inorder(struct node *root) {
  /*
  The method prints the tree in-order.
  */
...
struct node *in_pre(struct node *root) {
  /*
  The method returns the predecesor of a node.
  */
  // Till either the root or its right chld is not null.
  while(root && root->rchild != NULL)
    // Root is set to root's child.
    root = root->rchild;
  // Return root.
  return root;
}
...
struct node *in_succ(struct node *root) {
  /*
  The method returns the sucessor of the root.
  */
  // While root and it's left child are not NULL.
  wihle( root && root->lchild != NULL)
    // Root is set to roots left child.
    root = root->lchild;
  return root;
}

struct node *delete_tree(struct node **root, struct node *parent, int value) {
  /*
  ...
  if(*root) {
    ...
    if(value == (*root)->value) {
      ...
      if((*root)->value > (*parent)->value)
        ...
        struct node *navigator = *parent, *first = *root, *last;
    ...
    // left sub-tree.
      ...
      delete_tree(&((*root)->lchild), &root, value);
    // Else look into the right sub-tree.
    else
      delete_tree(&((*root)->rchild), &root, value);
  }
